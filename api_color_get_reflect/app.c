/**
 *  @file   app.c
 *  @brief  Check execution time of "ev3_color_sensor_get_reflect".
 *
...
{
    char msg_buf[32] = { 0 };
    char *logging_file_name = "ev3_color_sensor_get_reflect.csv";
    ...
    for (loop_index = 0; loop_index < 10; loop_index++) {
        ...
        reflect = ev3_color_sensor_get_reflect(sensor_port);    //First function call must be run before the sequence.
        ...
        fprintf(logging_file, "%d,%d,%d\n", loop_index, task_count, reflect);   //Write log data.
...
void spec_check_task(intptr_t unused)
{
    while (spec_check_task_running) {
        reflect = ev3_color_sensor_get_reflect(sensor_port);
        task_count++;
    }
}
