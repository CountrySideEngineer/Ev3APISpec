...
{
    char msg_buf[32] = { 0 };
    char *logging_file_name = "ev3_motor_get_counts.csv";
    ...
    for (loop_index = 0; loop_index < 10; loop_index++) {
        ...
        ev3_motor_set_power(motor_port, motor_power);   //To check the motor, start motor running.
        ...
        fprintf(logging_file, "%d,%d,%d\n", loop_index, task_count, get_motor_count);   //Write log data.

        ev3_motor_set_power(motor_port, 0); //Before next test step, stop the motor running.
...
void spec_check_task(intptr_t unused)
{
    while (spec_check_task_running) {
        get_motor_count = ev3_motor_get_counts(motor_port);
        task_count++;
    }
}
