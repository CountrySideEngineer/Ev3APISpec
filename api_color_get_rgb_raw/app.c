/**
 *  @file   app.c
 *  @brief  Check execution time of "ev3_color_sensor_get_rgb_raw".
 *
 *  @detail     Check spec of TOPPERS EV3RT.
 *              In this application, execution time of color sensor
 *              api "ev3_color_sensor_get_rgb_raw" is checked.
...
{
    char msg_buf[32] = { 0 };
    char *logging_file_name = "ev3_color_sensor_get_rgb_raw.csv";
    ...
    for (loop_index = 0; loop_index < 10; loop_index++) {
        ...
        ev3_color_sensor_get_rgb_raw(sensor_port, &rgb_raw);    //First function call must be run before the sequence.
        ...
        fprintf(logging_file, "%d,%d,%d,%d,%d\n",
            loop_index, task_count,
            rgb_raw.r, rgb_raw.g, rgb_raw.b);   //Write log data.
