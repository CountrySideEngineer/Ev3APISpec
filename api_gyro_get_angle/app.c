/**
 *  @file   app.c
 *  @brief  Check execution time of "ev3_gyro_sensor_get_angle".
 *
 *  @detail     Check spec of TOPPERS EV3RT.
 *              In this application, execution time of gyro sensor
 *              api "ev3_gyro_sensor_get_angle" is checked.
...
int16_t angle = 0;
static sensor_port_t sensor_port = EV3_PORT_3;
static sensor_type_t sensor_type = GYRO_SENSOR ;
...
{
    char msg_buf[32] = { 0 };
    char *logging_file_name = "ev3_gyro_sensor_get_angle.csv";
    ...
    for (loop_index = 0; loop_index < 10; loop_index++) {
        ...
        ev3_gyro_sensor_get_angle(sensor_port);    //First function call must be run before the sequence.
        ...
        fprintf(logging_file, "%d,%d,%d\n", loop_index, task_count, angle);   //Write log data.
...
void spec_check_task(intptr_t unused)
{
    while (spec_check_task_running) {
        angle = ev3_gyro_sensor_get_angle(sensor_port);
        task_count++;
    }
}
