/**
 *  @file   app.c
 *  @brief  Check execution time of "ev3_ultrasonic_sensor_listen".
 *
 *  @detail     Check spec of TOPPERS EV3RT.
 *              In this application, execution time of gyro sensor
 *              api "ev3_ultrasonic_sensor_listen" is checked.
...
{
    char msg_buf[32] = { 0 };
    char *logging_file_name = "ev3_ultrasonic_sensor_listen.csv";
    ...
    for (loop_index = 0; loop_index < 10; loop_index++) {
        ...
        listen = ev3_ultrasonic_sensor_listen(sensor_port);    //First function call must be run before the sequence.
        ...
        fprintf(logging_file, "%d,%d,%d\n", loop_index, task_count, (int)listen);   //Write log data.
...
void spec_check_task(intptr_t unused)
{
    while (spec_check_task_running) {
        listen = ev3_ultrasonic_sensor_listen(sensor_port);
        task_count++;
    }
}
