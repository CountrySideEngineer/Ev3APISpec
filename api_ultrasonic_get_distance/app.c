/**
 *  @file   app.c
 *  @brief  Check execution time of "ev3_ultrasonic_sensor_get_distance".
 *
 *  @detail     Check spec of TOPPERS EV3RT.
 *              In this application, execution time of gyro sensor
 *              api "ev3_ultrasonic_sensor_get_distance" is checked.
...
int16_t distance = 0;
static sensor_port_t sensor_port = EV3_PORT_1;
static sensor_type_t sensor_type = ULTRASONIC_SENSOR  ;
...
{
    char msg_buf[32] = { 0 };
    char *logging_file_name = "ev3_ultrasonic_sensor_get_distance.csv";
    ...
    for (loop_index = 0; loop_index < 10; loop_index++) {
        ...
        distance = ev3_ultrasonic_sensor_get_distance(sensor_port);    //First function call must be run before the sequence.
        ...
        fprintf(logging_file, "%d,%d,%d\n", loop_index, task_count, distance);   //Write log data.
...
void spec_check_task(intptr_t unused)
{
    while (spec_check_task_running) {
        distance = ev3_ultrasonic_sensor_get_distance(sensor_port);
        task_count++;
    }
}
