/**
 *  @file   app.c
 *  @brief  Check execution time of "ev3_motor_get_power".
 *
...
{
    char msg_buf[32] = { 0 };
    char *logging_file_name = "ev3_motor_get_power.csv";
    ...
    for (loop_index = 0; loop_index < 10; loop_index++) {
        ...
        fprintf(logging_file, "%d,%d,%d\n", loop_index, task_count, get_motor_power);   //Write log data.
...
void spec_check_task(intptr_t unused)
{
    while (spec_check_task_running) {
        get_motor_power = ev3_motor_get_power(motor_port);
        task_count++;
    }
}
